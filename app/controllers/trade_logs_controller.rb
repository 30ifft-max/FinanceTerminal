class TradeLogsController < ApplicationController
  TABS = %w[log holdings closed derivatives].freeze
  ...
  def index
    ...
    elsif @tab == "derivatives"
      @derivative_batches = DerivativeBatch
        .where(account_id: Account.accessible_by(Current.user).where(accountable_type: %w[Investment Crypto]).select(:id))
        .includes(:rounds, :account)
        .order(status: :asc, started_on: :desc)
    elsif @tab == "closed"
